...
__xs,ys,_xs_test,ys_test = training_data.spliterino(random_state=1)
_xs,xs_test = np.array(__xs),np.array(_xs_test)
print("data loaded")
...
print("Scaling")
xs = StandardScaler(copy=True).fit_transform(_xs) # remember it might be best to do this for the training data and never for the test set 😈
...
if True:
    ...
    dump(pre_clf, 'dynamic_svm_scaled.joblib')
